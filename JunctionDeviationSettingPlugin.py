...
class JunctionDeviationSettingPlugin(Extension):
    def __init__(self) -> None:
        ...
        self._i18n_catalog = i18nCatalog("junctiondeviationsettingplugin")
        ...
        if api_version < Version("7.3.0"):
            settings_definition_path = os.path.join(
                ...
                "junction_deviation35.def.json"
            )
        else:
            settings_definition_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)),
                "junction_deviation47.def.json"
            )
        ...
        try:
            ...
            Logger.logException("e", "Could not load junction deviation settings definition")
            return
    ...
    def _onContainerLoadComplete(self, container_id: str) -> None:
        ...
        try:
            speed_category = container.findDefinitions(key="speed")[0]
        except IndexError:
        ...
        for setting_key in self._settings_dict.keys():
            ...
            # which breaks stuff
            speed_category._children.append(setting_definition)
    ...
    def _filterGcode(self, output_device: "OutputDevice") -> None:
        ...
        if not global_container_stack.getProperty("junction_deviation_enable", "value"):
            return
        ...
        gcode_command_pattern = "M205 J%f"
        gcode_command_pattern += " ; JunctionDeviationSettingPlugin"
        ...
        for plate_id in gcode_dict:
            ...
            setting_key = "junction_deviation_factor"

            current_junction_deviation_factors = {}  # type: Dict[int, float]
            ...
            for extruder_stack in used_extruder_stacks:
                ...
                jd_factor = extruder_stack.getProperty(setting_key, "value")

                gcode_list[1] += gcode_command_pattern % (jd_factor) + "\n"

                dict_changed = True

                current_junction_deviation_factors[extruder_nr] = jd_factor

                apply_factor_per_feature[extruder_nr] = False
                for feature_setting_key in self.__gcode_type_to_setting_key.values():
                    if extruder_stack.getProperty(feature_setting_key, "value") != jd_factor:
                    ...
                    for line_nr, line in enumerate(lines):
                        ...
                        if line.startswith(";TYPE:"):
                            ...
                            if current_layer_nr <= 0 and feature_type != "SKIRT":
                                feature_setting_key = "junction_deviation_factor_layer_0"
                            ...
                            for extruder_stack in used_extruder_stacks:
                                ...
                                if feature_setting_key:
                                    jd_factor = extruder_stack.getProperty(feature_setting_key, "value")
                                else: # unknown feature type
                                    jd_factor = 0 # no JD for this feature

                                if jd_factor != current_junction_deviation_factors.get(extruder_nr, None):
                                    current_junction_deviation_factors[extruder_nr] = jd_factor

                                    lines.insert(line_nr + 1, gcode_command_pattern % (jd_factor))
    ...
    __gcode_type_to_setting_key = {
        "WALL-OUTER": "junction_deviation_factor_wall_0",
        "WALL-INNER": "junction_deviation_factor_wall_x",
        "SKIN": "junction_deviation_factor_topbottom",
        "SUPPORT": "junction_deviation_factor_support",
        "SUPPORT-INTERFACE": "junction_deviation_factor_support_interface",
        "SKIRT": "junction_deviation_factor_skirt_brim",
        "FILL": "junction_deviation_factor_infill",
        "PRIME-TOWER": "junction_deviation_factor_prime_tower"
    }
